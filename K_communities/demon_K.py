...
def Myclustering_with_K(adj_mat:np.array,T:int,L:int,seed:int,isself,gt, total_num_clusters):
    ...
    plot_heatmap_triangle(np.sign(U))
    # C_index = np.zeros(num_nodes)
    # num = 0
    # for data in list(np.sign(U)):
        
    #     sign_data = np.flip(np.sign(data))      # flip to meet the format of binary counting
    #     sign_data[sign_data==-1] = 0
        
    #     C_index[num] = 0
    #     for i in range(U.shape[1]-1):    # ignore the first eigenvector
    #         C_index[num] += sign_data[i]* 2**(i)
    #     num +=1
        
    # print(C_index)
    # print(Counter(C_index))
    # assert False
    ...
    plt.figure()
    plt.plot(pseudo_label)
    plt.savefig("result.jpg",dpi=300)

    eigenValues,eigenVectors = eign_computing(adj_mat,name="test")
    acc_cen_plus = centralized_Kmeans_plus(X = np.matmul(eigenVectors[:,:total_num_clusters],np.diag(eigenValues[:total_num_clusters])),
                                 gt = gt,num_clusters = total_num_clusters, init_centroids = first_centroids)   
    
    acc_cen_fixed = centralized_Kmeans_fixed(X = np.matmul(eigenVectors[:,:total_num_clusters],np.diag(eigenValues[:total_num_clusters])),
                                 gt = gt,num_clusters = total_num_clusters, init_centroids = first_centroids)   
    
    
    return acc_cen_fixed, acc_cen_plus ,acc_dec_rp,acc_dec_sign
...
if __name__ == '__main__':
    ...
    for index in range(realizations):
        ...
        acc_cen_fixed, acc_cen_plus ,acc_dec_rp, acc_dec_sign= Myclustering_with_K(adj_mat = adj_mat,T=30,L=30,seed=index,isself=False,gt = gt,
    ...
    print("Accuracy of decentralized spectral clustering with random initial",np.array(result_dec_rp).mean(),np.array(result_dec_rp).std())  
